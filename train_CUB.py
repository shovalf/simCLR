...
NNI = False
# if not NNI:
#     from simCLR import train_labels, train_features, test_labels, test_features
...
opt = {'gpu': '0', 'splitmode': 'easy', 'disp_interval': 20, 'save_interval': 200, 'evl_interval': 50,
               'manualSeed': None, 'resume': None, 'batchsize': 1000}
...
def train(_nni=False, train_labels=None, train_features=None, test_labels=None, test_features=None):
    param = _param()
    # if not _nni:
    #     dataset = LoadDataset(opt)
    # else:
    #     dataset = LoadDataset(opt, train_features, train_labels, test_features, test_labels)
    dataset = LoadDataset(opt,  train_labels, train_features, test_labels, test_features)
    ...
    exp_params = 'Eu{}_Rls{}_RWz{}'.format(CENT_LAMBDA , REG_W_LAMBDA, REG_Wz_LAMBDA)
    ...
    for it in range(start_step, num_epochs):
        """ Discriminator """
        for _ in range(5):
            ...
            z = Variable(torch.randn(batchsize, param.z_dim)).cuda()
        ...
        for _ in range(1):
            ...
            if CENT_LAMBDA != 0:
                ...
                Euclidean_loss *= 1.0/dataset.train_cls_num * CENT_LAMBDA
            ...
            if REG_Wz_LAMBDA != 0:
                ...
                reg_Wz_loss = Wz.pow(2).sum(dim=0).sqrt().sum().mul(REG_Wz_LAMBDA)
        ...
        if it % opt['disp_interval'] == 0 and it:
            ...
            log_text = 'Iter-{}; Was_D: {:.4}; Euc_ls: {:.4}; reg_ls: {:.4}; Wz_ls: {:.4}; G_loss: {:.4}; D_loss_real: {:.4};' \
                       ' D_loss_fake: {:.4}; rl: {:.4}%; fk: {:.4}%'\
                        .format(it, Wasserstein_D.data.item(),  Euclidean_loss.data.item(), reg_loss.data.item(),reg_Wz_loss.data.item(),
                                G_loss.data.item(), D_loss_real.data.item(), D_loss_fake.data.item(), acc_real * 100, acc_fake * 100)
            if not _nni:
                print(log_text)
            with open(log_dir, 'a') as f:
                f.write(log_text+'\n')
        ...
        if it % opt['evl_interval'] == 0 and it >= 100:
            netG.eval()
            acc = eval_fakefeat_test(it, netG, dataset, param, result, 3000+1, _nni=_nni)
            eval_fakefeat_GZSL(it, netG, dataset, param, result_gzsl, _nni=_nni)
...
def eval_fakefeat_test(it, netG, dataset, param, result, num_epochs, _nni=False):
    gen_feat = np.zeros([0, param.X_dim])
    for i in range(dataset.test_cls_num):
        text_feat = np.tile(dataset.test_text_feature[i].astype('float32'), (nSample, 1))
        text_feat = Variable(torch.from_numpy(text_feat)).cuda()
        z = Variable(torch.randn(nSample, param.z_dim)).cuda()
        G_sample = netG(z, text_feat)
    ...
    label_mat = (idx_mat[:, 0:Knn] / nSample).astype(int)
    ...
    if not _nni:
        print("{}nn Classifier: ".format(Knn))
...
""" Generalized ZSL"""
def eval_fakefeat_GZSL(it, netG, dataset, param, result, _nni=False):
    gen_feat = np.zeros([0, param.X_dim])
    for i in range(dataset.train_cls_num):
        text_feat = np.tile(dataset.train_text_feature[i].astype('float32'), (nSample, 1))
        text_feat = Variable(torch.from_numpy(text_feat)).cuda()
        z = Variable(torch.randn(nSample, param.z_dim)).cuda()
        G_sample = netG(z, text_feat)
        gen_feat = np.vstack((gen_feat, G_sample.data.cpu().numpy()))
    ...
    visual_pivots = [gen_feat[i*nSample:(i+1)*nSample].mean(0) \
...
#     with open("features/features_test__pre_ddp_0.pickle", 'rb') as handle:
#         test_features = pickle.load(handle).numpy().astype(np.float32)
#     with open("features/features_train__pre_ddp_0.pickle", 'rb') as handle:
#         train_features = pickle.load(handle).numpy().astype(np.float32)
#     with open("labels/labels_test__pre_ddp_0.pickle", 'rb') as handle:
#         test_labels = pickle.load(handle).numpy().astype(np.float32)
#     with open("labels/labels_train__pre_ddp_0.pickle", 'rb') as handle:
#         train_labels = pickle.load(handle).numpy().astype(np.float32)
#     train(_nni=NNI, train_labels=train_labels, train_features=train_features, test_labels=test_labels,
